...
def compare_in_batches(
    ...
    def check_memory() -> None:
        if monitor_memory:
            ...
            if mem['percent'] > max_memory_percent:
                log(f"Memory usage high ({mem['percent']:.1f}%), running garbage collection...")
                gc.collect()
    ...
    for batch_num, i in enumerate(range(0, len(all_columns), batch_size), 1):
        ...
        log(f"Processing batch {batch_num}/{total_batches} ({len(batch_columns)} columns)")
        check_memory()

        try:
            dc = DataCompare(
                base_df=base_path,
                compare_df=compare_path,
            ...
            log(f"Batch {batch_num} completed: Found {results.total_differences} differences")
            check_memory()
